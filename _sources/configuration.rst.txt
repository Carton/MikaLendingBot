...
- ``cancel_policy`` Controls which open loan offers the bot may cancel each cycle. Found in the ``[bot]`` section.
    ...
    - Default value: ``all``
    - Allowed values: ``all`` or ``limited``
    - ``all`` (legacy behavior): every open lending offer for active currencies is canceled before re-lending.
    - ``limited``: the bot maintains a rolling lending stake of up to ``max_offer_size``. Each cycle it (1) cancels the offers it created itself (they are re-placed at fresh rates), (2) re-reads the free balance, and (3) absorbs untracked offers — offers it did not create, e.g. manual placements or leftovers from another tool — to fund the cycle's target (``min(max_offer_size, max_active_amount headroom)``). Net absorption is bounded by ``max_offer_size`` per cycle: untracked offers that fit the gap are canceled whole (largest first); an oversized offer is *carved*: canceled, and its remainder — the part above the gap — is immediately re-placed at the **original rate and duration** (only its queue position is lost). Carved remainders are tracked as *preserved* and never refreshed by the bot. When a slice fills, the next cycle carves the next slice from the remaining funds, so parked money is converted slice by slice instead of being canceled in one shot. If a remainder is below ``min_loan_size`` the whole offer is absorbed (net absorption may exceed ``max_offer_size`` by up to ``min_loan_size`` in that case).
    - Use ``limited`` when you want the bot to manage a bounded stake while the rest of your funds sit in other offers: funds parked at other rates are left alone once the bot's stake is funded, but idle funds stuck in a single large offer can be freed automatically (carved) instead of requiring a manual cancel. Set ``max_active_amount`` if you want to cap how much of the sausage is eventually converted.
    - Failure behavior: if the bot cannot confirm the cancel of one of its own offers, it skips both absorption and lending for that currency for the cycle (no second slice can appear). If a cancel outcome is merely unknown (e.g. a timeout), absorption stops for the cycle and resumes after reconciliation. If the re-placement of a carved remainder fails, the remainder stays as free balance and is lent at the bot's current rate — the original rate and duration are not automatically restored; the same applies to a crash in the narrow window between a carve cancel and its re-placement.
    - Requirements: ``max_offer_size > 0`` and ``strategy = "FRR"`` for every active currency (currencies disabled via ``max_active_amount = 0`` are exempt); the bot refuses to start otherwise.
    - The offer registry (see ``offer_registry_file``) records which offers the bot created and which are preserved. With ``cancel_policy = "limited"``, a registry that is corrupt, has an unsupported version, or belongs to a different account pauses the bot's cancel and lending phases (with an error in the log) until the file is fixed or removed and the bot restarted — it never falls back to canceling everything. With the default ``all``, a registry problem only disables ownership recording; canceling and lending continue as before.

- ``offer_registry_file`` Path of the persistent offer registry used by ``cancel_policy = "limited"``. Found in the ``[bot]`` section.
