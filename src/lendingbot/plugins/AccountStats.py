...
DB_GET_TOTAL_EARNED = (
    "SELECT sum(earned) as total_earned, currency FROM 'history' GROUP BY currency"
)
DB_GET_YESTERDAY_EARNINGS = (
    "SELECT sum(earned) as total_earned, currency FROM 'history' "
    "WHERE close BETWEEN datetime('now', 'start of day', '-1 day') "
    "AND datetime('now','start of day') GROUP BY currency"
)
DB_GET_TODAYS_EARNINGS = (
    "SELECT sum(earned) as total_earned, currency FROM 'history' "
...
class AccountStats(Plugin):
    ...
    def check_upgrade(self) -> None:
        if self.db and 0 < self.get_db_version() < DB_VERSION:
            ...
            self.db.execute(DB_CREATE)
            self.db.commit()
            self.log.log(f"Upgraded AccountStats DB to version {DB_VERSION}")
    ...
    def update_history(self) -> None:
        ...
        self.fetch_history(self.api.create_time_stamp(last_time_stamp), time.time())
        ...
        if (self.get_db_version() == 0) and (self.get_first_timestamp() is not None):
            ...
            loop = True
            while loop:
                time.sleep(10)  # delay a bit, try not to annoy exchange
                first_time_stamp = self.get_first_timestamp()
                count = self.fetch_history(
                    self.api.create_time_stamp(last_time_stamp),
    ...
    def fetch_history(self, first_time_stamp: float, last_time_stamp: float) -> int:
        if not self.db:
            return 0
        history = self.api.return_lending_history(
            int(first_time_stamp), int(last_time_stamp - 1), 5000
        )
        loans = []
        for loan in history:
            loans.append(
        ...
        start_dt = datetime.datetime.fromtimestamp(first_time_stamp, datetime.UTC).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        end_dt = datetime.datetime.fromtimestamp(last_time_stamp - 1, datetime.UTC).strftime(
            "%Y-%m-%d %H:%M:%S"
        )

        self.log.log(f"Downloaded {count} loans history {start_dt} to {end_dt}")
        if count > 0:
            self.log.log(f"Last: {history[0]['close']} First:{history[count - 1]['close']}")
        return count

    def get_last_timestamp(self) -> Any:
        if not self.db:
            return None
    ...
    def get_first_timestamp(self) -> Any:
        if not self.db:
            return None
