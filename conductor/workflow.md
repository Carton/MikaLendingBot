...
3. **Write Failing Tests (Red Phase):**
   ...
   - **CRITICAL:** Run the tests using `uv run poe test` and confirm that they fail as expected. This is the "Red" phase of TDD. Do not proceed until you have failing tests.

4. **Implement to Pass Tests (Green Phase):**
   - Write the minimum amount of application code necessary to make the failing tests pass.
   - Run the test suite again using `uv run poe test` and confirm that all tests now pass. This is the "Green" phase.
...
6. **Verify Coverage & Quality:** 
   ...
   - **CRITICAL:** Run the linting and fixing command to ensure code quality:
     ```bash
     uv run poe fix-full
     ```
...
### Before Committing
```bash
# Perform full check (lint, format, type check)
